...
- 在 commit message / PR 描述中标注由哪个 subagent 完成、人工修改了哪些部分，commit message 必须包含有关变更的有效描述信息，在红–绿–重构推进开发的过程中，红、绿、重构应该放在三个不同的 Commit 中并通过 commit message 说明相关信息。
...
- 测试工具需要支持覆盖率统计和输出，在测试完成后打印测试报告，其中包含测试用例总数、通过数量以及测试覆盖率指标。
...
- 镜像须可通过 **单条 `docker build` + 单条 `docker run`** 完成构建与启动，并需要支持在本地通过 Makefile 来构建 (`make build`) 和启动 (`make run`)。
- README 中给出运行命令、端口、环境变量说明。
- 推送到 Docker Hub 或 GHCR，提供公开镜像地址，并保证可以在不登录 (`docker login`) 的情况下可以拉取镜像，镜像必须支持 x86 平台。
- CI 中包含镜像构建步骤。
